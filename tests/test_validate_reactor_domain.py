...
# SCPN Beam Target Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"configurations": ["beam_target", "beam_target"]},
            "must be unique",
        ),
        (
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-BEAM-TARGET-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-BEAM-TARGET-CORE"
                },
