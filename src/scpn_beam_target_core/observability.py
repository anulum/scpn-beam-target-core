...
# SCPN Beam Target Core — diagnostic and clock semantics
...
identities for the beam-target configurations this repository owns,
...
from scpn_beam_target_core.configuration import ConsistencyFinding
...
_CYCLIC_CLASSES: Final = frozenset({ObservabilityClass.DIRECT_CYCLIC})
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-BEAM-TARGET-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        phenomenon="accelerator cavity and bunch phase",
        configurations=("beam_target", "colliding_beam"),
        observability_class=ObservabilityClass.DIRECT_CYCLIC,
    ),
    CandidateProfile(
        candidate_id="beam.target_outcome",
        phenomenon="beam-target interaction and fusion-yield outcome",
        configurations=("beam_target", "colliding_beam"),
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-BEAM-TARGET-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-BEAM-TARGET-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented beam-target diagnostic practice;
        ...
        low, high = RF_BAND_HZ
        for channel in self.channels:
            if channel.candidate_id == "beam.rf_bunch_phase" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            "declared cavity/bunch band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the common accelerator RF "
                            "range 10 MHz-3 GHz (Wangler 2008)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the beam-target run scale of up to "
                            "~1000 s (Wangler 2008 context)"
                        ),
                    )
                )
