...
# SCPN Beam Target Core — device configuration container

"""Device configuration container bound to the SPO reactor registry.

A :class:`DeviceConfiguration` composes a validated beam line under
exactly one of the two registry identifiers this repository owns. The
beam-line count is a hard class invariant (one for a stationary target,
two for colliding beams); a centre-of-mass energy outside the documented
light-ion cross-section window is flagged (Bosch & Hale, NF 32 (1992)
611). Serialisation is canonical (sorted keys, no NaN or infinity
accepted anywhere) and the SHA-256 digest of those bytes identifies the
exact parameter set. The registry binding is a data pin only — this
package never imports SCPN Phase Orchestrator code.
"""
...
OWNED_CONFIGURATIONS: Final = ("beam_target", "colliding_beam")
BEAM_LINES_BY_IDENTIFIER: Final = {"beam_target": 1, "colliding_beam": 2}
CM_ENERGY_WINDOW_KEV: Final = (10.0, 300.0)
...
class DeviceConfiguration:
    """Validated beam-target device configuration.

    Parameters
    ----------
    identifier
        SPO registry configuration identifier; one of ``beam_target``
        or ``colliding_beam``.
    beam
        Validated beam line (both lines of a collider are symmetric).
    beam_line_count
        Number of beam lines; exactly one for ``beam_target`` and
        exactly two for ``colliding_beam``.
    registry
    ...
    DeviceConfigurationError
        ...
        beam-line count contradicts it.
    """
    ...
    def __post_init__(self) -> None:
        """Validate identifier ownership and the beam-line invariant.
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-BEAM-TARGET-CORE; owned: {OWNED_CONFIGURATIONS!r}"
            )
        expected = BEAM_LINES_BY_IDENTIFIER[self.identifier]
        if self.beam_line_count != expected:
            raise DeviceConfigurationError(
                f"beam_line_count: {self.identifier} requires exactly "
                f"{expected} beam line(s), got {self.beam_line_count!r}"
            )

    def centre_of_mass_energy_kev(self) -> float:
        """Nonrelativistic equal-mass centre-of-mass kinetic energy.

        Returns
        -------
        float
            ``E/2`` for a stationary target and ``2E`` for symmetric
            colliding beams (standard two-body kinematics).
        """
        if self.identifier == "beam_target":
            return self.beam.kinetic_energy_kev / 2.0
        return 2.0 * self.beam.kinetic_energy_kev
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            the centre-of-mass energy sits in the documented light-ion
            window. Findings are advisory instruments, not machine
            claims.
        """
        findings: list[ConsistencyFinding] = []
        energy = self.centre_of_mass_energy_kev()
        low, high = CM_ENERGY_WINDOW_KEV
        if not low <= energy <= high:
            findings.append(
                ConsistencyFinding(
                    field="beam.kinetic_energy_kev",
                    message=(
                        f"centre-of-mass energy {energy:.1f} keV is outside "
                        f"the documented light-ion cross-section window "
                        f"[{low:.0f}, {high:.0f}] keV"
                    ),
                )
            )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "beam": {
                "kinetic_energy_kev": self.beam.kinetic_energy_kev,
                "beam_current_ma": self.beam.beam_current_ma,
            },
            "beam_line_count": self.beam_line_count,
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "beam", "beam_line_count", "registry"}
    unknown = sorted(set(record) - known)
    if unknown:
        raise DeviceConfigurationError(f"record: unknown fields {unknown!r}")
    beam = _require_mapping(record, "beam")
    registry = _require_mapping(record, "registry")
    return DeviceConfiguration(
        identifier=_string(record, "identifier"),
        beam=BeamLine(
            kinetic_energy_kev=_number(beam, "kinetic_energy_kev"),
            beam_current_ma=_number(beam, "beam_current_ma"),
        ),
        beam_line_count=_integer(record, "beam_line_count"),
        registry=RegistryBinding(
