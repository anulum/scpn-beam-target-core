...
SCPN Beam Target Core — CONTROL adapter specification
-->

# CONTROL adapter specification

**Adapter identifier:** `scpn-beam-target-core.control-adapter`  
...
beam-target adapter would supply device truth to the SCPN control plane.
...
- channel identifier and physical quantity with SI unit — including
  beam-current, energy and emittance channels, luminosity or
  target-current accounting channels, reaction-product detector channels
  with centre-of-momentum kinematic accounting, and the mandatory paired
  loss channels (stopping and scattering) with exact definitions;
- coordinate convention (beamline coordinate, interaction-region frame)
  and spatial layout (scalar, per-bunch series, or detector array);
- sampling clock identity and timestamp semantics (continuous-run time
  base with declared bunch-level resolution);
...
- run lifecycle phase (`conditioning`, `beam_tuning`, `run`, `run_end`,
  `terminated`) with monotonic transition semantics and device-truth
  event records for beam loss, mis-steer, and target thermal damage;
- configuration identity: which registry configuration (`beam_target` or
  `colliding_beam`), which accelerator/target-station revision, and
  which configuration policy revision produced the run;
...
For each actuator class (source and accelerator programming, beam
transport and focusing configuration, target-station or interaction-region
systems, luminosity-management systems):
...
- declared capability envelope (bounds, slew limits, latency class) as
  ...
  lifecycle makes envelope-bounded slow adjustment declarable, strictly
  as declaration and never as a command surface;
- explicit marker `direct_actuation: false`.

### 4. Safety-envelope declaration (device → CONTROL and protection review)

- machine-readable operational envelope (beam power and loss bounds,
  target thermal limits, activation and shielding constraints with
  applicability domains);
- declared device-level hazard semantics for beam loss and target
  failure;
...
above through CONTROL's real admission path: nominal runs, beam-loss and
target-damage sequences, boundary-of-envelope cases, invalid-payload
